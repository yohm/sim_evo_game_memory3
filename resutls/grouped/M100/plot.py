...
# %%
x = [0.4,0.2,0.1, 0.04,0.02,0.01, 0.004,0.002,0.001, 0.0004,0.0002,0.0001, 0.00004,0.00002,0.00001]
...
def plot_mem_length_both(s_space, benefit, outfilepath=None):
  ...
  fig1, ax1 = plt.subplots()
  ax1.set_xscale("log")
  ax1.set_xlim(7e-6,1.0)
  ax1.set_ylim(-0.02, 1.02)
  ax1.set_xlabel("relative mutation probability, $r$")
  ax1.set_ylabel("memory length")
  if s_space[0] == 1:
    ax1.set_yticks([0, 0.5, 1.0])
    ax1.plot([0, 1.5], [0.5, 0.5], color='gray', linestyle='dotted')
  elif s_space[0] == 3:
    ax1.set_yticks([0, 1.5, 3])
    ax1.set_yticks([0.5,1.0,2.0,2.5], minor=True)
    ax1.plot([0, 1.5], [1.5, 1.5], color='gray', linestyle='dotted')
  else:
    raise ValueError("s_space[0] must be 1 or 3")
